...
```mermaid
flowchart LR
    accTitle: Конвейер Maccheroni
    accDescr: Аудио записывается и обрабатывается на Mac. Только необязательный путь Codex отправляет за пределы устройства ограниченные блоки текста.
    subgraph mac["Ваш Mac (аудио никогда не покидает устройство)"]
        A["Запись<br/>микрофон + системный звук"] --> B["Диаризация всего файла<br/>единая временная шкала говорящих"]
        A --> C["Листы ASR<br/>каждый максимум 120 с,<br/>глоссарий вводится для каждого листа"]
        B --> D["Объединение по временным меткам<br/>временная шкала определяет говорящих"]
        C --> D
        D --> E["Локальная постобработка<br/>исправление / перевод<br/>MLX на устройстве"]
    end
    D -.-> F["Путь Codex (необязательно)<br/>только ограниченные блоки текста,<br/>ваша подписка"]
...
| Постобработка (удалённая, только текст) | `gpt-5.6-sol` через CLI `codex` | управляется сервисом | не применимо |
...
- Необязательный путь постобработки Codex работает только с текстом и включается отдельно для каждого запуска. Он запускает `codex exec` в пустом временном рабочем пространстве с песочницей только для чтения и изоляцией пользовательской конфигурации; prompt содержит текст сегментов, активный глоссарий и инструкции. Если выбрать локальную модель MLX, даже текст останется на устройстве.
...
| `Tests/` | 147 тестов на основе fixtures в 16 наборах |
